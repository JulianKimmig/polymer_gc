...
from polymer_gc.data.dataset import PgDatasetConfig, Dataset, DatasetItem
...
class PolymerMaker:
    ...
    def _get_structures(self) -> List[Dict[str, SQLStructureModel]]:
        monomer_smiles: List[Dict[str, SQLStructureModel]] = []
        with SessionManager(db_path) as session:
            for smilespack in self.get_smiles():
                newpack = {}
    ...
    def _get_masses(self) -> Tuple[List[int], List[int], List[float]]:
        mn = self.rng.rand(N_POLYMERS) * (log_masses[1] - log_masses[0]) + log_masses[0]
        mn = np.exp(mn).astype(int)
        pdi = generate_pdi_distribution(
            N_POLYMERS, min_pdi, max_pdi, r=PDI_R, rng=self.rng
        )
    ...
    def _get_entry_graphs(self, entry: DatasetItem) -> List[ReusableGraphEntry]:
        samplemasses = np.clip(
            np.array(
                entry.sec.sample_masses(
                    n=GRAPHS_PER_POLYMER, random_state=int(entry.mn + entry.mw)
                )
            ),
            1,
            MAX_SAMPLE_MASS,
        ...
        for n, mass_idx, c in zip(
            *np.unique(n_nodes, return_counts=True, return_index=True)
            ...
            found_graphs.update([g.id for g in graphs])
    ...
    def __call__(self, dataset: Dataset, start: int = 0):
        structures = self._get_structures()
        mns, mws, pdis = self._get_masses()
        entries = []
        with SessionManager(db_path) as session:
            for i in tqdm(
                range(N_POLYMERS), desc=f"Generating {self.__class__.__name__} entries"
            ):
                entry = self._get_entry(i, mns[i], mws[i], structures[i], start=start)
                ...
                fig = visualize_polymer(
                    graphs[0].to_nx(), save_path=f"g_{self.__class__.__name__}_{i}.png"
                )
                #
...
class StartGradientCopolymer(GradientMixin, PolymerMaker):
...
class StartHomopolymer(HomopolymerMixin, StartGradientCopolymer):
    pass


class StartRandomCopolymer(RandomCopolymerMixin, StartGradientCopolymer):
    pass


class StartBlock(BlockMixin, StartGradientCopolymer):
    pass
